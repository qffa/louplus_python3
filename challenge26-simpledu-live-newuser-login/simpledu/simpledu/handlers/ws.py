...
def outbox(ws):
    chat.register(ws)
    client_count = len(set(chat.clients))
    welcome_msg = json.dumps({"username": "admin", "text": "new user come in, people count: {}".format(client_count)})
    redis.publish('chat', welcome_msg)
    while not ws.closed:
        gevent.sleep(0.1)
